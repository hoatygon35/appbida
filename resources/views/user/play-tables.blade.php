<x-app-layout>
    ...
    <div class="py-6 sm:py-8" x-data="playManager()">
        ...
        <div x-show="showServiceModal" class="fixed inset-0 z-50 flex items-start justify-center sm:pt-[10vh] bg-black/60 backdrop-blur-sm overflow-y-auto sm:p-4 max-md:p-0" x-cloak>
            <div class="bg-white max-w-2xl w-full overflow-hidden shadow-2xl transition-all transform flex flex-col sm:max-h-[85vh] sm:rounded-3xl max-md:h-full max-md:max-h-full max-md:rounded-none">
                                    ...
                                    <template x-for="(item, idx) in currentSessionServices" :key="idx">
                                        <tr class="hover:bg-slate-50/50 transition">
                                            ...
                                            <td class="py-2 px-3 text-center">
                                                <input type="number" x-model.number="item.pivot.quantity" min="1" @focus="$el.select(); setTimeout(() => $el.scrollIntoView({behavior: 'smooth', block: 'center'}), 150)" style="width: 50px;" class="text-center border border-slate-200 rounded-lg py-1 px-1 text-slate-800 font-bold focus:ring-2 focus:ring-indigo-400/20 focus:border-indigo-400 mx-auto inline-block shadow-sm">
                                            </td>
                                            <td class="py-2 px-3 text-center">
        ...
        <div x-show="showAddServiceModal" class="fixed inset-0 z-[60] flex items-start justify-center sm:pt-[10vh] bg-black/60 backdrop-blur-sm overflow-y-auto sm:p-4 max-md:p-0" x-cloak>
            <div class="bg-white max-w-2xl w-full overflow-hidden shadow-2xl transition-all transform flex flex-col sm:max-h-[85vh] sm:rounded-3xl max-md:h-full max-md:max-h-full max-md:rounded-none">
                        ...
                        <tbody class="divide-y divide-slate-100 text-xs font-medium text-slate-700">
                            <template x-for="item in filteredServices" :key="item.id">
                                <tr class="hover:bg-slate-50/50 transition">
                                    ...
                                    <td class="py-3 px-3">
                                        <input type="text" x-model="item.note" placeholder="Ghi chú..." @focus="setTimeout(() => $el.scrollIntoView({behavior: 'smooth', block: 'center'}), 150)"
                                    ...
                                    <td class="py-3 px-1.5 sm:px-3 text-center">
                                        <input type="number" x-model.number="item.qty" min="0" @focus="$el.select(); setTimeout(() => $el.scrollIntoView({behavior: 'smooth', block: 'center'}), 150)" style="width: 50px;" class="text-center border border-slate-200 rounded-lg py-1 px-0.5 sm:px-1 text-slate-800 font-bold focus:ring-2 focus:ring-indigo-400/20 focus:border-indigo-400 mx-auto inline-block shadow-sm">
                                    </td>
                                </tr>
                            </template>
                        </tbody>
